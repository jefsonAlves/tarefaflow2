...
async function startServer() {
  ...
  // API Routes for Google Classroom & Tasks
  // Note: Real OAuth flow would require clientID/Secret in .env
  ...
  app.post("/api/google/classroom/submissions", async (req, res) => {
    ...
    try {
      ...
      // If teacher, we can fetch all submissions using '-'. If student, only 'me'.
    ...
    try {
      ...
      const event = {
        ...
        start: {
          ...
          dateTime: new Date(new Date(task.dueDate).getTime() + 3600000).toISOString(), // 1 hour later
          timeZone: 'UTC',
        },
        colorId: task.completed ? '10' : (task.priority === 'high' ? '11' : '1'), // Green if completed, Red if high priority
      };
